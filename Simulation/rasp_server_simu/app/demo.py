...
def collectData():
    ...
    try:
        for meter_id in meters_port_map:
            print("http://" + meter_id + ":" + str(meters_port_map[meter_id]) + "/get_current")
            curVal += int(ast.literal_eval(requests.get("http://" + meter_id + ":" + str(meters_port_map[meter_id]) + "/get_current").text))
            print(curVal)
        glob_current = curVal
