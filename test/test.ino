void setup()
{
  String InputMessage = "current=56;usage=78";
  Serial.begin(9600);
  Serial.println();
...
void ProcessInputMessage(String InputString)
{
  ...
  //Print the result
  res+=PrintWithLeadingZero(h0);
  res+=PrintWithLeadingZero(h1);
  res+=PrintWithLeadingZero(h2);
  res+=PrintWithLeadingZero(h3);
  res+=PrintWithLeadingZero(h4);
  res+=PrintWithLeadingZero(h5);
  res+=PrintWithLeadingZero(h6);
  res+=PrintWithLeadingZero(h7);
  Serial.println();
  Serial.print("----------------------------------------------------------------");
  Serial.println();
  Serial.println(res);
...
String PrintWithLeadingZero(unsigned long ReceiveUnsignedLong)
{
  ...
  TempLz = ReceiveUnsignedLong & 0xf0000000;
  TempLz = TempLz >> 28;
  Serial.print(TempLz, HEX);
  Serial.println(" adad" + getHex(TempLz));
  s += getHex(TempLz);
  
  TempLz = ReceiveUnsignedLong & 0x0f000000;
  TempLz = TempLz >> 24;
  Serial.print(TempLz, HEX);
  s += getHex(TempLz);
  
  TempLz = ReceiveUnsignedLong & 0x00f00000;
  TempLz = TempLz >> 20;
  Serial.print(TempLz, HEX);
  s += getHex(TempLz);
  
  TempLz = ReceiveUnsignedLong & 0x000f0000;
  TempLz = TempLz >> 16;
  Serial.print(TempLz, HEX);
  s += getHex(TempLz);
  
  TempLz = ReceiveUnsignedLong & 0x0000f000;
  TempLz = TempLz >> 12;
  Serial.print(TempLz, HEX);
  s += getHex(TempLz);

  
  TempLz = ReceiveUnsignedLong & 0x00000f00;
  TempLz = TempLz >> 8;
  Serial.print(TempLz, HEX);
  s += getHex(TempLz);
  
  TempLz = ReceiveUnsignedLong & 0x000000f0;
  TempLz = TempLz >> 4;
  Serial.print(TempLz, HEX);
  s += getHex(TempLz);
  
  TempLz = ReceiveUnsignedLong & 0x0000000f;
  TempLz = TempLz >> 0;
  Serial.print(TempLz, HEX);
  s += getHex(TempLz);
  Serial.println();
  Serial.println("res:" + s);
  return s;
}
